...
import 'package:quiz_quest/app/features/quiz_pages/sports_quiz_pages/hard_sport_quiz_page/hard_question_quiz_page.dart';
...
class HardSportAnswerButton extends StatefulWidget {
  ...
  @override
  State<HardSportAnswerButton> createState() => _HardSportAnswerButtonState();
}

class _HardSportAnswerButtonState extends State<HardSportAnswerButton> {
  void onPressed() {
    if (isButtonDisabled) {
      return;
    }
    ...
    if (widget.isCorrectAnswer) {
      ...
      hardSportGoodAnswers += 1;
    } else {
      widget.colorFunction(Colors.red, widget.index);
      hardSportBadAnswers += 1;
      if (hardSportBadAnswers == 3) {
        context.read<SportCubit>().updateHardSportsPoints(hardSportGoodAnswers);
        context
            .read<RankingCubit>()
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) =>
                HardSportLostLifePage(goodAnswers: hardSportGoodAnswers),
          ),
        );
      }
    }
